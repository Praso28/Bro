    ...
    with open(file_path, "rb") as f:
        ...
        for line in lines:
            ...
            if city not in city_map:
                city_map[city] = [Decimal('Infinity'), Decimal(0), Decimal('-Infinity'), 0]
            stats = city_map[city]
    ...
    for city_map in results:
        for city, stats in city_map.items():
            if city not in final_map:
                final_map[city] = [Decimal('Infinity'), Decimal(0), Decimal('-Infinity'), 0]
...
def process_file():
    ...
    for city in sorted(final_results.keys()):
        ...
        mean_val = (stats[1] / Decimal(stats[3])).quantize(Decimal('0.1'), rounding=ROUND_CEILING)
